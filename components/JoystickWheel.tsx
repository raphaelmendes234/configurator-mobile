...
export function JoystickWheel({
  ...
  const createSlicePath = (
    ...
    const circleRect = Skia.XYWHRect(
      center.x - radius - bonusSize * 0.5, // Ajout de la size bonus pour faire dépasser la roue d'arrière plan (et créer des bordures)
      center.y - radius - bonusSize * 0.5,
      radius * 2 + bonusSize,
      radius * 2 + bonusSize,
    );
  ...
  const strokePath = useDerivedValue(() => {
    ...
    for (let i = 0; i < parts; i++) {
      // On saute l'index sélectionné
      if (i !== selectedIndex.value) {
        const p = createSlicePath(i, center, sliceAngle, effectiveSweep, 8);
        path.addPath(p);
      }
    }
    return path;
  ...
  const transformOrigin = useDerivedValue(() => {
    return { x: fingerPosX.value, y: fingerPosY.value };
  ...
  return (
    ...
    >
      <Group origin={transformOrigin} transform={transform}>
